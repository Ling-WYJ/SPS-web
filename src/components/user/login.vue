...
<template>
  <div class="login-wrapper">
    ...
    >
      ...
      <!-- 线上版本登录方式 -->
      <el-form-item prop="userID">
        <el-input v-model="form.user_name" placeholder="请输入用户名" type="text" clearable></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$refs.login.validate((valid) => {
        if (valid) {
          this.$ajax.post('/auth', this.form).then((res) => {
            if (res.data) {
              this.login(res.data)
            } else {
              this.$message({
    ...
    login(data) {
      ...
      this.tim
          ...
          .then(async() => {
            ...
            await sessionStorage.setItem('GET_USER_INFO',JSON.stringify({
              userID: this.form.user_name,
              userSig: window.genTestUserSig(this.form.user_name).userSig,
              sdkAppID: window.genTestUserSig('').SDKAppID,
              role: data.role
